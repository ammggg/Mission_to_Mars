...
def scrape_all():
    ...
    data = {
        ...
        "scrape_data": hemisphere_images(broswer)
        }
...
def hemispheres_images(browser):
    ...
    browser.visit(url)
    html = browser.html
    urls_to_scrape = soup(html, 'html.parser')
    items = []
    
    try:
        for a in urls_to_scrape.find_all('a', class_ = 'itemLink'):
    ...
    try:
        for url_i in urls_to_open:
            ...
            img_url = url_soup.select('.downloads ul li a')[0].get('href')
            img_title = url_soup.select('.cover h2')[0].text
